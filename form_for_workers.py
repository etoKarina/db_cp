...
class Example(tk.Frame):
    ...
    def delete_product_submenu(self):
        ...
        if height:
            self.chosen_product_id = StringVar()
            self.chosen_product_id.set('')
            for i in range(height):  # Rows
                tk.Label(self.submenu_frame, text=products[i][1]).grid(row=i + 1, column=0)
                tk.Radiobutton(self.submenu_frame, variable=self.chosen_product_id, value=products[i][0]).grid(row=i + 1, column=2)
            tk.Button(self.submenu_frame, text='Удалить', command=self.delete_products).grid(row=height + 1, column=1)
        else:
            self.clear_submenu()
    ...
    def delete_products(self):
        ...
        propagate_error_to_ui(self.restoraunt.delete_product)(self.chosen_product_id.get())